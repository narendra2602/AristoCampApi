...
public interface ComboDataDao extends JpaRepository<MktDataDto, Integer> {
	...
	@Query(value = "select depo_code code,depo_name name from crm.branch_comp order by depo_name ", nativeQuery = true)
	List<ComboDataDto> getBranchList(@Param("userId") int userId);

	@Query(value = "select id code,camp_name name from campmaster where div_code in (select div_code from userdiv where user_id=:userId and user_status='Y' order by div_code) ", nativeQuery = true)
